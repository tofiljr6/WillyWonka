...
router.post('/add_supplier', function (req, res) {
  if (req.session.type === 'manager' || req.session.type === 'admin') {
    try {
      controller.addSupplier(req.session.type, req.body.name, req.body.nip);
      res.redirect('/');
    } catch (e) {
      console.log(e);
    }
...
router.post('/add_client', function (req, res) {
  if (req.session.login) {
    try {
      controller.addClient(req.session.type, req.body.name, req.body.city, req.body.street);
      res.redirect('/');
    } catch (e) {
      console.log(e);
    }
...
router.get('/add-chocolate', function (req, res) {
  if (req.session.type === 'manager' || req.session.type === 'admin') {
    res.render('add-chocolate', {
      nick: req.session.login
    });
  } else if (req.session.login) {
    res.redirect('/');
